...
const aiLabels = ['AI generated'];

const realLabels = [
  'Real image (not generated by AI)',
...
export async function checkIfAI(url) {
  ...
  const classifications = await _hfInterface.zeroShotImageClassification({
    ...
    parameters: {
      candidate_labels: [...realLabels, ...aiLabels],
    },
  });

  // Check if the AI generated classification is above 0.5
  const aiClassifications = classifications.filter((c) =>
    aiLabels.includes(c.label)
  );
